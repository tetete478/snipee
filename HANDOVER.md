...
| 変数                                              | 出力例                | 説明                   |
| ------------------------------------------------- | --------------------- | ---------------------- |
| `{今日:MM/DD}`                                    | `12/07`               | 今日の日付             |
| `{明日:MM/DD}`                                    | `12/08`               | 明日の日付             |
| `{2日後:M月D日:曜日短（毎月1日は除外して3日後）}` | `12月9日（月）`       | 2 日後（1 日除外）     |
| `{3日後:M月D日:曜日短（毎月1日は除外して4日後）}` | `12月10日（火）`      | 3 日後（1 日除外）     |
| `{タイムスタンプ}`                                | `2025/12/07 14:30:45` | 年月日＋時分秒         |
| `{名前}`                                          | `小松`                | 設定画面で登録した名前 |
...
**Mac**: ✅ Apple Developer Program 対応済み
...
- permission-guide.html をスクロール可能に
- コンテンツをコンパクト化（パディング・フォントサイズ調整）
- 「ClipyWindows」→「Snipee」に表記修正

**Windows ワンクリックインストール:**

- NSIS インストーラーをワンクリックモードに変更
- 完了画面なしで即起動

**手動アップデート再起動修正:**

- 設定画面からの「再起動」ボタンに `app.isQuitting = true` を追加
- `quitAndInstall(false, true)` で確実に再起動

### v1.5.26（2025-12-07）

**Mac 自動アップデート再起動修正:**

- `app.isQuitting = true` を追加してウィンドウクローズを許可
- `quitAndInstall(false, true)` で強制再起動
- 再起動ボタンが正常に動作するように

**履歴使用時の最新移動:**

- ペーストした履歴アイテムを自動的に先頭に移動
- 一般的なクリップボードアプリと同じ挙動に
- タイムスタンプも更新
...
- カスタム `notarize.js` を削除
- electron-builder 内蔵の notarize に移行
- `afterSign` 設定を削除
- `notarize: { teamId: "F8KR53ZN3Y" }` に変更
- Mac ビルドが正常に完了するように（無限ハング問題解消）
...
```bash
rm -rf ~/Library/Application\ Support/snipee
rm -rf ~/Library/Caches/snipee
